...
const { readArgNames, ArgNameToken } = require('../app');
 ...
 * to asynchronous code blocks set via the startClient and startServer
...
class IPCService {
    ...
    set startClient( fn ) {
        if( typeof fn != 'function' ) {
            throw new Error('IPCService: startClient must be a function');
        }
        this._startClient = fn;
    }

    set startServer( fn ) {
        if( typeof fn != 'function' ) {
            throw new Error('IPCService: startServer must be a function');
        }
        this._startServer = fn;
    }
     ...
     */
    get client() {
        // Return an async lambda to make the client instance.
        // This is done to allow the _startClient arg list to be exposed
        ...
        function make() {
            if( !self._client ) {
                ...
                await self._startClient.invoke( this, arguments );
        ...
        make[ArgListToken] = readArgNames( this._startClient );
        return make;
    }
    ...
    async get server() {
        // Return an async lambda to make the client instance.
        // This is done to allow the _startServer arg list to be exposed
        // as a property of the make function.
        const self = this;
        function make() {
            if( !self._server ) {
                // Note that 'self' refers to the IPCService instance, whilst 'this'
                // will refer to the app backbone when invoked by the backbone.
                await self._startServer.invoke( this, arguments );
                const serverID = self._id+'-server';
                self._server = await self._makeServer('remote', serverID );
            }
            return self._server;
        }
        make[ArgListToken] = readArgNames( this._startServer );
        return make;
    }
    ...
    async get service() {
        ...
        function make() {
            if( !self._service ) {
                ...
                // will refer to the app backbone when invoked by the backbone.
                await self._startServer.invoke( this, arguments );
                self._service = await self._makeServer('local', self._id );
            }
