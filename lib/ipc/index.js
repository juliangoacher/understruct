...
class IPCService {
    ...
    get client() {
        if( !this._client ) {
            ...
            const client = new Client( clientID, serverID, this._logger, this._appspace );
    ...
    get server() {
        if( !this._server ) {
            const serverID = this._id+'-server';
            this._server = this._makeServer('remote', serverID );
        }
        return this._server;
    }
    ...
    get service() {
        if( !this._service ) {
            this._service = this._makeServer('local', this._id );
        }
